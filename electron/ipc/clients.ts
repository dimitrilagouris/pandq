...
export function registerClientHandlers(): void {
  ...
  ipcMain.handle('db-create-client', (_event, payload: CreateClientPayload) => {
  ...
  ipcMain.handle('db-update-client', (_event, payload: UpdateClientPayload) => {
    const { id, name, businessName, email, phone, address } = payload;
    const db = getDb();
    const oldClient = db.prepare('SELECT * FROM clients WHERE id = ?').get(id) as { name: string; business_name: string; email: string; phone: string; address: string } | undefined;
    db.prepare(
  ...
  ipcMain.handle('db-delete-client', (_event, id: number) => {
    const db = getDb();
