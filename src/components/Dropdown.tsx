...
export const Dropdown: React.FC<DropdownProps> = ({
  ...
  return (
    <div className={variant === 'input' ? 'w-full relative' : 'relative inline-block text-left'} ref={containerRef}>
      ...
      {isOpen && (
        <div className={`dropdown-menu-open absolute z-50 mt-1 bg-stone-600/95 backdrop-blur-md border border-white/5 rounded-2xl shadow-22 p-1.5 flex flex-col gap-0.5 animate-in fade-in slide-in-from-top-2 duration-150 ${alignmentClass} ${widthClass}`}>
          {options.map((opt) => (
            <button
              key={opt.value}
              type="button"
              ...
              className="w-full px-2.5 py-1.5 text-left text-xs font-regular text-white hover:bg-white/10 rounded-lg transition-colors capitalize border-0 bg-transparent cursor-pointer"
            >
              {opt.label}
            </button>
          ))}
